...
def dudoan():
	...
	with open(link) as mylink:
		...
		if inference[0] == 0:
			tileloss = "Dự đoán: tỉ lệ loss: ",(round(loss, 2)),"%", "và tỉ lệ win: ",(round(won, 2)),"%"
			t5.set(tileloss)
		elif inference[0] == 1:
			tilewin = "Dự đoán: tỉ lệ win: ",(round(won, 2)),"%", "và tỉ lệ loss:" , (round(loss, 2)),"%"
			t5.set(tilewin)
		else:
    			print(inference)
...
savebtn = Button(wrapper1, text= "Sava Data", command=savedb)
...
exbtn = Button(wrapper1, text= "Exit", command=lambda: exit())
...
up_btn = Button(wrapper2, text="Update", command=update_customer)
add_btn = Button(wrapper2, text="Add New", command=add_new)
delete_btn = Button(wrapper2, text="Delete", command=delete_customer)
